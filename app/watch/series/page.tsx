...
import { series } from "@/lib/messages";
      ...
      <section style={{ background: "#fff", padding: "clamp(64px,8vw,110px) clamp(20px,4vw,40px)" }}>
        <div style={{ maxWidth: 1340, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 22 }}>
            {series.map((s) => (
              <Link key={s.name} href="/watch" className="step-card" style={{ display: "block", textDecoration: "none", color: "inherit", borderRadius: 12 }}>
                <div style={{ position: "relative", borderRadius: 12, overflow: "hidden", aspectRatio: "1 / 1", boxShadow: "0 1px 3px rgba(11,24,48,.1)" }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={s.cover} alt={s.name} className="step-img" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
